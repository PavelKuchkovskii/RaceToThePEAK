...
- Three host-configurable campfire progression policies: wait for nobody, wait for each team independently, or wait for the whole lobby.
...
The same host panel provides three waiting policies for every respawn mode:

1. **Wait for nobody**: the first living racer activates the campfire and loads the next biome for everyone. Other racers do not need to claim the same transition. Final rising hazards use separate player clocks.
...
- Your individual timer pauses while your selected progression scope still needs to complete a campfire, and resumes when that player/team/lobby completes it.
