...
* Fixed hidden Mega Launch food consumption on PEAK 2.0 by detecting the first completed bite, including the per-portion `Action_ReduceUses` path used by multi-use food, before the delayed item RPC. Guest consumption now uses an owner-authenticated request that the host validates against the marked item and its current holder.
